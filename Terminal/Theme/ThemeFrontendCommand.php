...
Dev auto-setup (no manual .env required):
  ...
  - Injects VITE_* into npm run dev at runtime (VITE_SERVER_URL, VITE_DEV_PROXY, …)
  - Does not modify theme .env by default — use --sync-env to persist an autogenerated block to .env.local
  - Copy .env.example → .env.local once with --init-env (or fe scaffold)
...
Development (.env.local overrides, optional):
  ENV_SERVER_SYNC=true   persist autogenerated VITE_* block on each fe dev (opt-in)
  VITE_DEV=true
  VITE_DEV_SERVER=http://127.0.0.1:5173

HMR: @pinooxhq/vite-plugin writes theme/.pinoox/dev.json; PHP mirrors into project .pinoox/dev.json.
FOOTER
            ))
            ...
            ->addOption('sync-env', null, InputOption::VALUE_NONE, 'Persist autogenerated VITE_* block to theme .env.local on dev start')
            ->addOption('init-env', null, InputOption::VALUE_NONE, 'Copy .env.example to .env.local when missing (no overwrite)')
            ->addOption('env-file', null, InputOption::VALUE_REQUIRED, 'Theme env file for --sync-env / ENV_SERVER_SYNC (default: .env.local)')
    ...
    private function runInfo(SymfonyStyle $io, ThemeFrontend $frontend): int
    {
        ...
        $io->definitionList(
            ...
            ['Theme .env' => !empty($info['env_autodev'])
                ? (string) ($info['env_file'] ?? '.env.local') . ' (auto block present)'
                : (string) ($info['env_file'] ?? '.env.local') . ' — runtime inject only; use fe dev --sync-env or ENV_SERVER_SYNC=true in .env.local to persist'],
    ...
    private function runDev(
        ...
        $this->applyDevEnvOptions($frontend, $input);
    ...
    private function runDevStack(
        ...
        foreach ($devTargets as $devTarget) {
            ...
            $this->applyDevEnvOptions($frontend, $input, $envFile);
    ...
    private function runScaffold(SymfonyStyle $io, ThemeFrontend $frontend, string $stack): int
    {
        ...
        FrontendDevSync::initThemeEnv($frontend->themePath());
    ...
    private function applyDevEnvOptions(ThemeFrontend $frontend, InputInterface $input, ?string $envFile = null): void
    {
        if ($envFile !== null && trim($envFile) !== '') {
            $frontend->setDevEnvFile(trim($envFile));
        } else {
            $envFileOption = $input->getOption('env-file');
        ...
        $frontend->setSyncDevEnv((bool) $input->getOption('sync-env'));
        $frontend->setInitDevEnv((bool) $input->getOption('init-env'));
    }
