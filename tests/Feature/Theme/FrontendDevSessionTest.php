...
test('FrontendDevSession builds locked serve URLs at PHP origin without app-router path', function () {
    ...
    expect($session->phpAppUrl)->toBe('http://127.0.0.1:19800')
        ->and($session->displayAppUrls())->toBe(['http://127.0.0.1:19800'])
        ->and($session->proxyPrefixes)->toContain('/api')
        ->and($session->proxyPrefixes)->not->toContain('/manager')
        ->and($session->viteDevServerUrl())->toBe('http://127.0.0.1:' . $session->vitePort)
        ->and($session->viteDevPortLabel())->toBe((string) $session->vitePort);
});
...
JS);
    ...
    $config = FrontendConfig::forThemePath($themePath);
    $session = FrontendDevSession::fromOptions('com_demo_app', $config, '127.0.0.1', 19800, 'com_demo_app', true);
    $corePath = dirname(__DIR__, 3);

    $result = FrontendDevSync::sync($themePath, $config, $corePath, $session, true, '.env.local', true);

    expect($result['env_autodev'])->toBeTrue()
        ->and($result['env_file'])->toBe('.env.local')
        ...
        ->and(file_get_contents($themePath . '/vite.config.js'))->toContain('pinooxDevState')
...
test('FrontendDevSync merges autodev block into existing .env without wiping custom keys', function () {
    ...
    file_put_contents($themePath . '/frontend.config.php', "<?php\n\nreturn ['stack' => 'vue'];\n");
    file_put_contents($themePath . '/.env.local', "MY_CUSTOM_KEY=keep-me\nENV_SERVER_SYNC=true\n");
    ...
    $env = file_get_contents($themePath . '/.env.local');

    expect($env)->toContain('MY_CUSTOM_KEY=keep-me')
        ->and($env)->toContain('VITE_SERVER_URL=http://127.0.0.1:19800')
        ->and($env)->toContain('VITE_BUILD_OUT_DIR=dist')
...
JS);
    ...
    expect($config)->toContain('@pinooxhq/vite-plugin')
        ->and($config)->toContain('pinooxDevState')
...
test('FrontendDevSession npmEnvironment respects theme env values over auto resolution', function () {
    ...
    expect($npmEnv)->not->toHaveKey('VITE_DEV_PORT')
        ->and($npmEnv['VITE_SERVER_URL'])->toBe('http://127.0.0.1:19800');
});
