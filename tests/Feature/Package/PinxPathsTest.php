...
it('uses pinx workspace paths for keys and releases', function () {
    ...
    expect(PinxPaths::defaultKeyRelative())->toBe('pinx/keys/sign.key.json')
        ...
        ->and(str_replace('\\', '/', PinxPaths::releasesDir($appPath)))->toBe(str_replace('\\', '/', $appPath) . '/pinx/releases');
    ...
    expect(str_replace('\\', '/', $release))->toStartWith($normalizedAppPath . '/pinx/releases/com_test_paths_v3_')
        ->and(str_ends_with($release, '.pinx'))->toBeTrue()
        ->and(is_dir(PinxPaths::releasesDir($appPath)))->toBeTrue();

    @unlink($release);
    @rmdir(PinxPaths::releasesDir($appPath));
...
it('falls back to legacy pinx key and export directories', function () {
    ...
    $legacyRelease = PinxPaths::legacyExportDir($appPath) . '/legacy.pinx';
    file_put_contents($legacyRelease, 'test');

    expect(PinxPaths::collectReleaseFiles($appPath))->toBe([$legacyRelease]);
