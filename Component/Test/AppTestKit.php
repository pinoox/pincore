...
class AppTestKit
{
    ...
    {
        return AppPackagePath::fromTestsFile($file);
    }
    ...
    {
        return SystemConfig::corePath('tests');
    }

    public static function fixturesRoot(): string
    {
        return self::testsRoot() . '/Fixtures';
    }

    public static function fixturesProjectRelative(string $relative = ''): string
    {
        $absolute = self::fixturesRoot();
        if ($relative !== '') {
            $absolute .= '/' . ltrim(str_replace('\\', '/', $relative), '/');
        }
